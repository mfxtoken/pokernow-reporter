            ...
            <div className="mb-6 border-b border-gray-200 dark:border-gray-700">
              <nav className="flex gap-2">
                ...
                <button
                  ...
                  className={`px-4 py-3 font-medium transition-colors flex items-center gap-2 border-b-2 ${activeTab === 'players'
                    ? 'border-purple-600 text-purple-600 dark:text-purple-400'
                    : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200'
                    }`}
                >
            ...
            {activeTab === 'players' && (
              <div ref={reportRef} className="space-y-8 bg-white dark:bg-gray-900 p-4 rounded-xl">
                ...
                {(() => {
                  ...
                  const sortedPlayers = Object.values(playerStats).sort((a, b) => b.totalNet - a.totalNet);
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {sortedPlayers.map(player => {
                          const netInDollars = player.totalNet / 100;
                          const isPositive = netInDollars > 0;
                          const isNegative = netInDollars < 0;

                          return (
                            <div
                              key={player.name}
                              className={`p-6 rounded-xl shadow-lg transition-transform hover:scale-105 ${isPositive
                            ...
                            >
                              ...
                              <div className="space-y-2">
                                <div className="flex justify-between">
                                  <span className="opacity-90">Games:</span>
                                  <span className="font-semibold">{player.gamesPlayed}</span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="opacity-90">Net:</span>
                                  <span className="font-bold text-xl">
                                    {isPositive ? '+' : ''}₹{netInDollars.toFixed(2)}
                                  </span>
                                </div>
                                <div className="flex justify-between text-sm">
                                  <span className="opacity-75">Buy-in:</span>
                                  <span>₹{(player.totalBuyIn / 100).toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                  <span className="opacity-75">Buy-out:</span>
                                  <span>₹{(player.totalBuyOut / 100).toFixed(2)}</span>
                                </div>
                              </div>
                            </div>
                          );
